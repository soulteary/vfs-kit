...
func (m *Mounter) Umount(point string) error {
	...
	for ii, v := range m.points {
		if v.point == point {
			// Check if we have mount points strictly below this one
			for _, vv := range m.points[ii:] {
				rel, ok := hasSubdir(v.point, vv.point)
				if ok && rel != "" {
...
// Remove removes the path. Note: currently empty directories may be removed
// even if a mount point exists below; this behavior may be restricted in a future version.
